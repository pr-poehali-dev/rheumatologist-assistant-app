...
export default function BodyJointMap({ onJointsChange }: BodyJointMapProps) {
  ...
  const handleJointClick = (joint: Joint, e: React.MouseEvent<SVGCircleElement>) => {
    ...
    setPopupPos({ x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 });
  };
  ...
  return (
    <div className="relative body-map-container">
      ...
      <div className="relative flex justify-center">
        ...
        {/* Popup — fixed, умное позиционирование */}
        {activeJoint && popupPos && (() => {
          const popupH = 280;
          const screenH = window.innerHeight;
          const spaceBelow = screenH - popupPos.y;
          const showBelow = spaceBelow >= popupH + 16;
          const top = showBelow ? popupPos.y + 16 : popupPos.y - popupH - 16;
          const left = Math.min(Math.max(popupPos.x - 128, 8), window.innerWidth - 264 - 8);
          return (
            <>
              <div className="fixed inset-0 z-40" onClick={closePopup} />
              <div
                className="fixed z-50 bg-white rounded-2xl shadow-2xl border border-orange-100 p-4 w-64 animate-slide-up"
                style={{ top, left }}
              >
                {/* Title */}
